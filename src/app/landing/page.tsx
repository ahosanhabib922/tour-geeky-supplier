    ...
    <div className="bg-white text-brand-black">
      ...
      {(over.hero_show !== false) && (
        <header className="py-20 lg:py-28 px-6 sm:px-12 max-w-6xl mx-auto text-center space-y-8 relative overflow-hidden">
          <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-brand-light border border-brand-border text-[11px] font-bold uppercase tracking-wider text-brand-black style-hero_badge">
            <Award className="w-3.5 h-3.5 text-brand-black" /> {over.hero_badge || "Operator Onboarding Hub"}
          </div>
          
          <h1 className="text-[clamp(2.5rem,5vw,4.5rem)] font-medium tracking-tight text-brand-black max-w-4xl mx-auto leading-[1.05] tracking-[-0.02em] whitespace-pre-line style-hero_title">
            {over.hero_title || "Grow Your Activity Business with\nTour Geeky Partner"}
          </h1>
          
          <p className="text-[15px] sm:text-[17px] text-brand-gray max-w-2xl mx-auto font-medium leading-relaxed style-hero_description">
          ...
          <div className="pt-4 flex flex-col sm:flex-row items-center justify-center gap-4">
            <Button 
              onClick={openModal}
              size="lg"
              className="w-full sm:w-auto rounded-full px-10 h-12 text-sm font-bold bg-brand-black text-white hover:bg-brand-black/90 shadow-md flex items-center justify-center gap-2 style-hero_ctaText"
            >
        ...
        <section id="benefits" className="py-24 bg-brand-light/20 border-y border-brand-border px-6 sm:px-12">
          <div className="max-w-6xl mx-auto space-y-16">
            <div className="text-center space-y-3">
              <h2 className="text-2xl sm:text-3xl font-bold tracking-tight text-brand-black style-benefits_title">{over.benefits_title || "Why Partner with Us?"}</h2>
              <p className="text-sm text-brand-gray font-medium style-benefits_subtitle">{over.benefits_subtitle || "Everything you need to list, manage, and scale Greece experiential business."}</p>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              {benefitsList.map((b: any, idx: number) => {
                ...
                return (
                  <div key={idx} className={`bg-white border border-brand-border/40 p-8 rounded-[28px] space-y-5 hover:shadow-xl hover:shadow-brand-black/5 hover:-translate-y-1 transition-all duration-300 style-benefit_card_${idx}`}>
      ...
      {(over.stats_show !== false) && (
        <section id="stats" className="py-24 max-w-6xl mx-auto px-6 sm:px-12 space-y-16">
          <div className="text-center space-y-3">
            <h2 className="text-2xl sm:text-3xl font-bold tracking-tight text-brand-black style-stats_title">{over.stats_title || "Our Greece Marketplace Impact"}</h2>
            <p className="text-sm text-brand-gray font-medium style-stats_subtitle">{over.stats_subtitle || "Greece's premier channel for premium experiential excursions."}</p>
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {statsList.map((s: any, idx: number) => (
              <div key={idx} className={`p-6 rounded-[24px] bg-white border border-brand-border/40 hover:shadow-xl hover:shadow-brand-black/5 transition-all duration-300 space-y-3 style-stat_card_${idx}`}>
      ...
      {(over.workflow_show !== false) && (
        <section id="workflow" className="py-24 bg-brand-light/20 border-y border-brand-border px-6 sm:px-12">
          <div className="max-w-6xl mx-auto space-y-16">
            <div className="text-center space-y-3">
              <h2 className="text-2xl sm:text-3xl font-bold tracking-tight text-brand-black style-workflow_title">{over.workflow_title || "How It Works"}</h2>
              <p className="text-sm text-brand-gray font-medium style-workflow_subtitle">{over.workflow_subtitle || "Simple 4-step workflow from partner registration to payout."}</p>
            </div>

            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
              {stepsList.map((s: any, idx: number) => (
                <div key={idx} className={`space-y-4 relative group style-step_card_${idx}`}>
      ...
      {(over.stories_show !== false) && (
        <section id="stories" className="py-24 max-w-6xl mx-auto px-6 sm:px-12 space-y-16">
          <div className="text-center space-y-3">
            <h2 className="text-2xl sm:text-3xl font-bold tracking-tight text-brand-black style-stories_title">{over.stories_title || "Greek Operator Stories"}</h2>
            <p className="text-sm text-brand-gray font-medium style-stories_subtitle">{over.stories_subtitle || "Hear from verified travel operators running experiences on Tour Geeky Greece."}</p>
          </div>

          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            {storiesList.map((st: any, idx: number) => (
              <div key={idx} className={`p-8 sm:p-10 rounded-[32px] border border-brand-border/40 bg-white hover:shadow-xl hover:shadow-brand-black/5 transition-all duration-300 flex flex-col justify-between gap-8 style-story_card_${idx}`}>
      ...
      {(over.faq_show !== false) && (
        <section id="faqs" className="py-24 bg-brand-light/20 border-t border-brand-border px-6 sm:px-12">
          <div className="max-w-4xl mx-auto space-y-12">
            <div className="text-center space-y-3">
              <h2 className="text-2xl sm:text-3xl font-bold tracking-tight text-brand-black style-faq_title">{over.faq_title || "Frequently Asked Questions"}</h2>
              <p className="text-sm text-brand-gray font-medium style-faq_subtitle">{over.faq_subtitle || "Have questions about payouts, listings, or commissions?"}</p>
            </div>

            <div className="space-y-4">
              {faqsList.map((f: any, idx: number) => (
                <div key={idx} className={`bg-white border border-brand-border/40 p-6 rounded-[20px] space-y-2.5 hover:shadow-xl hover:shadow-brand-black/5 transition-all style-faq_card_${idx}`}>
