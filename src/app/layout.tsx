...
import { LayoutDashboard, Box, ClipboardList, Wallet, Plane, Moon, X, Menu, Bell, Search, Sun } from "lucide-react";
import "./globals.css";
    ...
    <html lang="en" className="h-full antialiased bg-white text-zinc-950">
      <body className="min-h-full flex flex-col font-sans selection:bg-brand-black selection:text-white">
        <div className="flex h-screen overflow-hidden">
          ...
          {/* Mobile Top Header */}
          <div className="lg:hidden fixed top-0 left-0 right-0 h-16 bg-white border-b border-brand-border z-30 flex items-center justify-between px-6">
            <Link href="/" className="flex items-center gap-3">
              <div className="w-8 h-8 bg-brand-black rounded-xl flex items-center justify-center">
                <Plane className="w-4 h-4 text-white" />
              </div>
              <span className="font-bold text-base tracking-tight text-brand-black">Tour Geeky</span>
            </Link>
            <button onClick={() => setIsSidebarOpen(true)} className="p-2 text-brand-gray hover:bg-brand-light rounded-full">
              <Menu className="w-6 h-6" />
            </button>
          </div>

          {/* Sidebar Layout */}
          <aside className={`w-[280px] border-r border-brand-border/60 bg-white flex flex-col justify-between h-full fixed lg:relative z-40 transform transition-all duration-300 ease-in-out shrink-0 ${
              ...
              <div className="h-20 flex items-center px-8 border-b border-brand-border/60 shrink-0">
                <Link href="/" className="flex items-center">
                  ...
                  <div className="ml-3 flex flex-col animate-in fade-in duration-300">
                    <span className="font-bold text-[15px] tracking-tight leading-none text-brand-black">Tour Geeky</span>
                    <span className="text-[9px] text-brand-gray uppercase font-bold tracking-wider mt-1.5">Supplier Hub</span>
                  </div>
                </Link>
                <button className="ml-auto lg:hidden p-2 text-brand-gray hover:bg-brand-light rounded-full" onClick={() => setIsSidebarOpen(false)}>
                  <X className="w-5 h-5" />
                </button>
              </div>
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <div className="relative">
                    <div className="h-8.5 h-8.5 rounded-full bg-brand-light flex items-center justify-center font-bold text-brand-black border border-brand-border text-xs">
                      AS
                    </div>
          ...
          <main className="flex-1 flex flex-col min-w-0 bg-white h-screen overflow-hidden mt-16 lg:mt-0">
            ...
            {/* Sticky Top Header */}
            <header className="h-[80px] shrink-0 border-b border-brand-border/60 flex items-center justify-between px-6 lg:px-10 bg-white/85 backdrop-blur-md z-10 sticky top-0">
              <div className="flex items-center gap-6">
                <div className="hidden sm:flex items-center gap-1.5 text-[11px] text-brand-gray font-bold uppercase tracking-wider">
              ...
              <div className="flex items-center gap-4">
                <div className="relative hidden md:flex w-[220px]">
            ...
            <div className="flex-1 overflow-y-auto p-6 sm:p-10 lg:p-12 bg-white relative">
              {children}
            </div>
          </main>
          ...
          {isSidebarOpen && (
            <div 
              className="fixed inset-0 bg-brand-black/20 z-30 lg:hidden backdrop-blur-sm transition-all duration-300" 
              onClick={() => setIsSidebarOpen(false)} 
            />
          )}
        </div>
